...
def sequence_callback(msg):
    ...
    for step in plan:
        ...
        status_pub.publish(f"started:{action}")
        rospy.loginfo(f"[execute_control] (SIM) Starting meta-action {action}")

        for primitive in sequence:
            status_pub.publish(f"started:{primitive}")
            rospy.loginfo(f"[execute_control] (SIM) Executing {primitive}")
            try:
                simulate()
                status_pub.publish(f"completed:{primitive}")
            except Exception as e:
                rospy.logerr(f"[execute_control] (SIM) Primitive {primitive} failed: {e}")
                status_pub.publish(f"failed:{primitive}")
                status_pub.publish(f"failed:{action}")
                return

        status_pub.publish(f"completed:{action}")
        rospy.loginfo(f"[execute_control] (SIM) Completed meta-action {action}")
...
def main():
    ...
    rospy.loginfo("[execute_control] SIMULATION mode enabled.")
    rospy.spin()
