...
class SignalCoordinator:
    ...
    def evaluate_conditions(self):
        rospy.sleep(0.2)  # Allow all statuses to update
...
class WizardAgent:
    def __init__(self):
        ...
        # Load assistant and thread
        self.assistant_id = rospy.get_param("~assistant_id", "")
    ...
    def load_context(self):
        config_base = rospy.get_param("/config_path", "../config")
        with open(f"{config_base}/high_task.json") as f:
            task_data = json.load(f)
        with open(f"{config_base}/behavior_high.json") as f:
            behavior_data = json.load(f)
        with open(f"{config_base}/object_aruco_high.json") as f:
    ...
    def query_agent(self, msgs):
        ...
        prompt += "\nShould the robot be listening now? Return either 'start_listen' or 'stop_listen'."
    ...
    def publish_decision(self, signal: str):
        if signal in ["start_listen", "stop_listen"]:
            self.listen_pub.publish(signal)
            rospy.loginfo(f"WizardAgent published to /listen_signal: {signal}")
        else:
            rospy.logwarn(f"WizardAgent received invalid response: {signal}")
